...
const Modals = {
	...
	RedBlackTree: (
		<ul>
			<li>A self-balancing BST where every node is colored red or black.</li>
			<li>
				<strong>Properties:</strong> the root is always black; red nodes must have black
				children; every path from a node to a null leaf has the same black-height.
			</li>
			<li>
				<strong>Insert:</strong> New nodes are always inserted as red. Red-red violations
				are fixed via recoloring or rotations (fixDoubleRed).
			</li>
			<li>
				<strong>Delete:</strong> Removing a black node creates a &quot;double-black&quot;
				deficit, resolved by recoloring or rotations (fixExtraBlack).
			</li>
			<li>
				<strong>Time complexity:</strong> O(log n) guaranteed for insert, delete, and
				search — not amortized.
			</li>
			<li>
				<strong>vs AVL:</strong> RB trees do fewer rotations on insert/delete but may be
				slightly less balanced. Preferred when writes are frequent.
			</li>
			<li>
				<strong>Real-world use:</strong> Java TreeMap/TreeSet, C++ std::map, Linux kernel
				CFS scheduler.
			</li>
		</ul>
	),
